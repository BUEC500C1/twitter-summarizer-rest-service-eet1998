...
def image2video(handle):
    ...
    file_name = os.getcwd() + '/' + handle + '_tweets'
    video_name = handle + 'video/' + handle + '_video.mp4'
    out, err = ffmpeg.input(file_name + '*.png', pattern_type = 'glob', framerate = 0.3).filter('fps', fps=1).output(video_name).run()
    return out
